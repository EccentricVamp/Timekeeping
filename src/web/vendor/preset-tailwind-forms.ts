// Copied from https://github.com/tw-in-js/twind/blob/dd0bb84ce8b0a7819b874ed781951d7a0ce245f9/packages/preset-tailwind-forms/src/preset-tailwind-forms.ts
// License MIT
// TODO: Replace this once https://esm.sh or https://twind.dev fix the actual plugin

import type { CSSObject, MaybeThunk, Preflight, Preset } from "twind";
    ...
    : {
      preflight(context) {
        ...
        for (const { b: base, s: styles } of rules) {
          preflight["" + base] = typeof styles == "function"
            ? styles(context)
            : styles;
        }
...
function svgToDataUri(svgString: string): string {
  return (
    "data:image/svg+xml," +
    encodeURIComponent(svgString.trim().replace(/\s+/g, " ").replace(/"/g, "'"))
      .replace(
        /%[\dA-F]{2}/g,
        specialHexEncode,
      )
